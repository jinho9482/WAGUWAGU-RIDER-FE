import { Alert, Clipboard, Image, Linking, Platform, StyleSheet, Text, ToastAndroid, TouchableOpacity, View } from 'react-native';
// import Clipboard from '@react-native-clipboard/clipboard';
import WebView from 'react-native-webview';
import { useEffect, useState } from 'react';
import * as Location from 'expo-location';
import { changeActivationState } from '../api/Rider';
import { MoveDeliveryRequestToPostgres, deleteDeliveryRequest, getDeliveryRequests } from '../api/DeliveryRequest';
import { updateOrderState, updateOrderStateToMongo, updateOrderStateToRedis } from '../api/Order';
...
export default function HomeScreen({ navigation }) {
    ...
    const [deliveryButtonText, setDeliveryButtonText] = useState("픽업 완료 & 배달 시작");
    ...
    const clickOnActivationButton = async () => {
        ...
        if (riderId) {
            if (activationText === "활성화") {
                ...
                const res = await getDeliveryRequests(riderId, {latitude: location.coords.latitude, longitude: location.coords.longitude});
    ...
    const handleMessage = async (event) => {
        // 주문 도메인에 배달 수락 요청
        const res = await updateOrderStateToRedis(orderedItem.orderId, "배달요청");
        ...
        if (res) {
            ...
            Alert.alert("알람", "이미 배정이 완료되었습니다. 다른 요청건을 확인해주세요");
            // 요청 목록에서 제거 필요
    ...
    const updateDeliveryState = async () => {
        ...
        if (deliveryButtonText === "픽업 완료 & 배달 시작") {
          ...
          await updateOrderStateToRedis(orderedItem.orderId, "배달중");
        //   await updateOrderStateToMongo(orderedItem.orderId, "배달중");
          setDeliveryButtonText("배달완료"); 
        } 
        else {
          // 주문 상태 -> 배달 완료로 업데이트 필요 (주문 도메인에서)
          await updateOrderStateToRedis(orderedItem.orderId, "배달완료");
          await updateOrderStateToMongo(orderedItem.orderId, "배달완료");
          ...
          await MoveDeliveryRequestToPostgres(await AsyncStorage.getItem("riderId"), orderedItem.deliveryRequestId);
          // 비활성화 버튼 다시 활성화
