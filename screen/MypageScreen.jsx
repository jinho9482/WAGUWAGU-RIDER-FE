...
const MyPage = ({ navigation }) => {
    ...
    const fetchRiderInfo = async () => {
      try {
        ...
        if (res.status === 200) {
          ...
          const areas = res.data.activityAreas.map(
            (area) => area.riderActivityArea
          );
  ...
  const handleSaveActivityAreas = async () => {
    try {
      ...
      const deletePromises = activityAreas.map((area) =>
        deleteActivityArea({ riderActivityArea: area })
      );
      ...
      if (allDeleted) {
        ...
        const savePromises = activityAreas.map((area) =>
          saveActivityArea({ riderActivityArea: area })
        );
          ...
          <View style={styles.activityAreaContainer}>
            ...
            {activityAreas.map((area, index) => (
              <View key={index} style={styles.activityItem}>
                ...
                {modify && (
                  <TouchableOpacity
                    onPress={() => handleRemoveActivityArea(index)}
                  >
